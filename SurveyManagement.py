...
class SurveyManagement:
    ...
    def show_help(self):
        ...
        <html><head><style>
          ...
          <p><b>Version:</b> 1.1.0 &nbsp;|&nbsp; <b>Copyright:</b> 2026 ASTROMAT GEO-SERVICES</p>
          ...
          and multi-user access control.</p>

          <h2>✨ Features</h2>
          <ul>
            <li>📋 Survey metadata management with 36-state dropdown</li>
            <li>📍 Direct coordinate input &amp; plotting</li>
            <li>📐 Traverse calculator — WCB &amp; quadrant bearings, closure error with 1:5000 ratio</li>
          ...
          <h2>👥 User Roles</h2>
          <ul>
            <li><b>Superuser</b> — full access, create/disable users, view audit log</li>
          ...
          <ol>
            ...
            <li>Disable or change the default admin password</li>
          </ol>
    ...
    def run(self):
        ...
        # 0. First-run wizard — show if database has never been set up
        from qgis.core import QgsSettings
        # Clear old Gemini API key — AI extraction now uses hosted server,
        # no individual API key needed
        ...
        if not setup_done:
            try:
                ...
                wizard.exec_()
                # Refresh setup_done flag after wizard
                setup_done = QgsSettings().value("survey_management/setup_complete", "")
            except Exception as e:
                print(f"[SurveyMgmt] Wizard error: {e}")

        # 1. Ensure psycopg2 — re-check at runtime, not just at module load
        # (wizard may have just installed it during this session)
        psycopg2_ok = False
        try:
            import psycopg2 as _pg  # noqa
            psycopg2_ok = True
        except ImportError:
            # Try lib/ folder (wizard installs there)
        ...
        # If dialog already open and user is logged in, just bring it forward
        ...
        login = LoginDialog(self.iface.mainWindow(), db_connection=db_connection)
    ...
    def _on_dialog_closed(self):
        self.dialog = None
        # Keep session_user so re-opening doesn't need another login this session
